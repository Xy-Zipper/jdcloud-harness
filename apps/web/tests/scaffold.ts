...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  try {
    ...
    // Node need not resolve the browser-only remoteAuthority: connect to the
    // bound loopback server while preserving the authority whose cookie and
    // request-trust behavior the browser scenario exercises.
    const tokenExchangeUrl = options.remoteAuthority === undefined
      ? authenticatedUrl
      : authenticatedUrl.replace(baseUrl, `http://127.0.0.1:${String(port)}`)
    const login = await fetch(tokenExchangeUrl, {
      redirect: 'manual',
      ...options.remoteAuthority === undefined
        ? {}
