...
Session Controller 会在持久化附件和投递 inbox 前运行 `api-session/prompt-admission`。
...
| 拒绝 Web Prompt 或模型选择 | 监听 `api-session/prompt-admission` 或 `api-session/model-selection-admission`；调用 `next()` 放行 |
