...
A `ToolSchema` (the model-facing fields) plus a mandatory canonical output declaration, the `execute` function, an optional request-specific schema projector, host-only scheduler metadata, optional content callbacks, and optional UI presenters. The registry holds these; the loop dispatches calls through them. The registry's `schemas()` builds the model-facing `ToolSchema[]` by an explicit allowlist — `output`/`modelSchema`/`execute`/`projectContent`/`finalizeContent`/`timeoutMs`/`isConcurrencySafe`/`presentCall`/`presentResult` must never leak into a model request.
