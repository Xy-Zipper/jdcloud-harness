...
浏览器 `prompt()` 入口会在请求级模型校验后、附件持久化、source 创建、queue mutation 或 Agent 投递前运行 `api-session/prompt-admission` waterfall。listener 调用 `next()` 放行 Prompt，或通过拒绝让提交内容完全留在 Session 与 Agent 生命周期之外。该 hook 有意只面向浏览器 Prompt；它不替代 `agent/pre-step`，后者管理 inbox 已领取的消息。
