...
`selectModel()` 会先解析目标普通 Agent，再于模型校验、Session 修改或部署默认值持久化之前运行 `api-session/model-selection-admission`。拒绝监听器会保持 Session 选择与 `agentDefaultModel` 不变；接受监听器必须调用 `next()`。
