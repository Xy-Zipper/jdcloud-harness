...
The browser `prompt()` entry runs the `api-session/prompt-admission` waterfall after request-level model validation but before attachment persistence, source creation, queue mutation, or Agent delivery. A listener calls `next()` to admit the prompt or rejects it to leave the submitted content outside the Session and Agent lifecycle. This hook is intentionally browser-prompt-specific; it does not replace `agent/pre-step`, which governs messages after the inbox claims them.
