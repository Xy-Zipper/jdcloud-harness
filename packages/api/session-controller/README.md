...
`selectModel()` resolves the addressed ordinary Agent, then runs `api-session/model-selection-admission` before model validation, Session mutation, or deployment-default persistence. A rejecting listener leaves both the Session selection and `agentDefaultModel` unchanged; an accepting listener must call `next()`.
