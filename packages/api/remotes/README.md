...
Credential reference and record updates are separate forwarded events. `credentials/reference-updated` identifies environment-style secret references; `credentials/record-updated` identifies owner-defined records such as the JDCloud login token, allowing an optional Client feature to refresh redacted state without exposing the stored payload.
