...
当已挂载的执行器约束命令（例如 `dsh-bash-sandbox`）时，被阻止的文件操作会报告为 `[sandbox: file access denied under <mode> mode]`——这是策略拒绝，不是命令失败。审批可用时，请求 schema 只公开严格宽于会话当前模式的档位，模型可以用 `sandbox_permissions` 加一句 `justification` 重试一次完全相同的命令；该重试引发的审批提示就是用户同意的方式。无法进行获批的更宽调用时，schema 与拒绝结果都会省略升权指导。升权绝不能预先推测：没有真实拒绝依据的请求，或没有严格宽于当前模式的请求，会在不运行任何东西的情况下失败关闭，被拒绝的升权对该命令即为最终结果。
