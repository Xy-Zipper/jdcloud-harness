...
挂载策略插件后，`write` 与 `edit` 从 `fs/*` 意图槽位取得防护，因此未读目标或陈旧观察会以 `FS_NOT_OBSERVED` 或 `FS_STALE_VERSION` 及恢复指令失败。使用施加沙箱限制的后端（`fs-sandbox`）时，`write`/`edit` 仅在会话可以请求严格更宽的模式时公开 `sandbox_permissions` 与 `justification`。被拒绝的变更始终返回 `[sandbox: file access denied under <mode> mode]` 标记，仅在升权可执行时追加同轮次提示，并可在该次调用中加盖获批的更宽模式。
