...
With the policy plugin mounted, `write` and `edit` obtain their guard from the `fs/*` intent slots, so an unread target or a stale observation fails with `FS_NOT_OBSERVED` or `FS_STALE_VERSION` and a recovery instruction. Under a confining backend (`fs-sandbox`), `write`/`edit` advertise `sandbox_permissions` and `justification` only when the session can ask for a strictly wider mode. A denied mutation always returns the `[sandbox: file access denied under <mode> mode]` marker, adds the same-turn escalation hint only when actionable, and may stamp an approved wider mode for that one call.
