...
The bundle places each tenant-user's new files under `/workspace/jdcloud-users/<owner-hash>`; `JDCLOUD_USER_WORKSPACE_DIR` can override the persistent parent. Existing host-home directories are not migrated. Directory browsing, Workspace registration, and Session creation stay under the current user's directory, but terminal commands and absolute-path file reads remain unrestricted.
The bundle disables the internal-testing notice and automatic first-run API-key dialog; the Models settings page and manual API-key entry remain available.
