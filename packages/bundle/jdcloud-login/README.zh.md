...
该 bundle 将每个租户用户的新文件放在 `/workspace/jdcloud-users/<归属哈希>` 下；`JDCLOUD_USER_WORKSPACE_DIR` 可覆盖持久化父目录。旧的 Host 主目录不迁移。目录浏览、工作区登记和会话创建限制在当前用户目录内，但终端命令和通过完整路径读取文件仍不受此限制。
该 bundle 不显示内测声明及首次运行自动弹出的 API Key 对话框；模型设置页与手动填写 API Key 的入口保持可用。
