...
`ctx.sidebarRightTabs.registerAvailabilityFilter(filter)` applies a reversible browser policy to page kinds. A rejected kind is absent from registry lookups, the guide, tab rendering, and direct navigation while its registration remains intact. The filter does not authorize Host operations or delete an existing layout record.
