...
`src/client/contract.ts` defines contribution, decoration, and reversible availability-filter registration. `CommandDirectory` owns the per-session wire cache and resolves typed commands through `resolution.ts`; that module owns first-party identity matching and localized input spellings. Forwarded `commands/change` events soft-invalidate entries, while `connection/reset` hard-invalidates them; epoch guards prevent superseded pulls from publishing stale results. `matchSpace` reads the ready cache synchronously, while `matchEnter` waits on the SubmitAttempt signal and rejects on warmup failure or cancellation. After a matched Host execution, this browser emits `command/executed`; other clients observe only the durable command events. `PopupSelectController` owns popup state, and `PopupSelectView` occupies the input overlay. `presentation.ts` owns row labels, icons, and sections; its helpers and the resolution helpers stay internal to the plugin.
