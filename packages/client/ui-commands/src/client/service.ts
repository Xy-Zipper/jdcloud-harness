...
export class CommandUiRuntime extends Service implements CommandUiContract {
  ...
  private readonly live: LiveState = {
    contributions: new Map(), decorations: new Map(), availabilityFilters: new Set(), popups: new WeakMapWithValues(),
  }
  ...
  private async matchEnter(
    ...
    const unavailable = (): never => {
      throw new Error(this.t('notice.commandUnavailable', { command: typedName }))
    }
    ...
    if (contribution !== undefined && contribution.available(session)) {
      if (!this.isAvailable(typedName, session)) unavailable()
    ...
    if (!this.isAvailable(name, session)) unavailable()
