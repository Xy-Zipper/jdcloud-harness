...
Set `browserAuthentication: false` when deployment-owned access control replaces the Harness launch token. The root page then opens without a token, but Connection still mints and requires an independent `dsh-user-session` cookie so Host state cannot fall back to one process-global login. Host/Origin validation remains active and returns 403 for untrusted or cross-site requests; a missing browser identity returns 401. An application login mounted inside the Web profile does not by itself protect Host endpoints that are callable before login.

Deployments that need independent browser login without Harness launch-token authentication can enable `browserSession`. It issues an HttpOnly `dsh-user-session` cookie, carries its identity through each request, and lets Host services key login state by browser rather than by one process-global record.
