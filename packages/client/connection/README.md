...
Set `browserAuthentication: false` only when deployment-owned access control replaces the Harness browser session. The root page then opens without a token, and trusted requests reach every Host API and WebSocket without a cookie. Host/Origin validation remains active and returns 403 for untrusted or cross-site requests, but it does not identify a user. An application login mounted inside the Web profile does not by itself protect Host endpoints that are callable before login.
