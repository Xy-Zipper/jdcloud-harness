...
export class BrowserSessionService extends Service {
    ...
    constructor(ctx, secret, maxAgeMilliseconds) {
     ...
     */
    static async create(ctx, credentials, maxAgeDays = MAX_AGE_DAYS) {
    ...
    authorizeIndex(request, response) {
        if (this.browserId(request) !== undefined)
            return true;
    ...
    run(request, operation) {
        ...
        if (browserId === undefined)
            throw new Error('browser user session is required');
    ...
    currentIdRequired() {
        ...
        if (browserId === undefined)
            throw new Error('browser user session is unavailable outside a request');
        return browserId;
    }
    browserId(request) {
        ...
        if (payload === undefined || payload.authority !== requestAuthority(request.headers))
            return undefined;
        const now = Date.now();
...
function cookieValue(value, name) {
    for (const segment of value.split(';')) {
        ...
        if (at >= 0 && segment.slice(0, at).trim() === name)
...
function decodeCookie(value, secret) {
    ...
    if (body === undefined || encoded === undefined || extra !== undefined)
        return undefined;
...
async function loadSecret(credentials) {
    ...
    if (record?.kind !== 'grant' || typeof record.payload !== 'object' || record.payload === null)
        throw new Error('browser session secret is invalid');
    const value = Reflect.get(record.payload, 'secret');
