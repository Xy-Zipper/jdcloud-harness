 ...
 * slashes, `..` segments, and symlinks are all resolved. Workspace records
 * store this canonical spelling, and attach-time session `cwd` checks use the
 * same canon. Multiple owner-scoped registrations may share one canonical
 * path. Relative paths reject before `realpath` can resolve them from the Host
 * cwd or current Windows drive. A path that does not exist rejects with the
 * original `ENOENT` — this is `create`'s reject path (a workspace must point at
