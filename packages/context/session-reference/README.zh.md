...
- **Host 持有的授权**：未提供租户-用户归属查询的宿主，仍负责授权 `ctx.sessionQuery` 公开的会话；该服务不是面向模型的搜索工具。
